...
const setNfcs = async (req, res) => {
  try {
    ...
    await pool.query(
      "CREATE TABLE nfcs (nfc_id bigint PRIMARY KEY, was_swiped BOOLEAN DEFAULT false);"
    );
...
const getNfcStatusById = async ({ params: { id } }, response) => {
  try {
    await pool.query(
      `SELECT was_swiped FROM nfcs WHERE nfc_id = '${parseInt(id)}'`,
      (error, results) => {
        if (error) {
          throw error;
        }
...
const updateSwipedNfc = async (request, response) => {
  try {
    ...
    await pool.query(
      `UPDATE nfcs SET was_swiped = true WHERE nfc_id = '${id}'`,
      (error, results) => {
        if (error) {
          throw error;
        }
